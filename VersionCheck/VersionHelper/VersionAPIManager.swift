...
internal class VersionAPIManager {
    
    /// Constants used for the iTunes Lookup API request.
    struct Constant {
        static let iTunesURL = "http://itunes.apple.com/lookup?id=\(Utilities.Constants.appID)"
    }
    
    // MARK: - Variable Declaration
    static let shared = VersionAPIManager()
    ...
    func getLookUpData(completion: @escaping (LookUpResponseModel?, Error?) -> Void) {
        guard let identifier = Bundle.bundleIdentifier(),
            let url = URL(string: "\(Constant.iTunesURL)&bundleId=\(identifier)") else {
